...
def create_organization(request_body, project_id):
    ...
    db.session.commit()
    # Organization
    organization_data = {k: request_body[k] for k in ("organization_name", "cif", "nrc", "org_type", "vat_mode")}
...
def update_organization(request_body, organization_id):
    ...
    # Organization
    organization_data = {k: request_body[k] for k in ("organization_name", "cif", "nrc", "vat_mode")}
...
def get_all_suppliers(owner_id):
    return Organization.query.filter_by(org_type=organization_type.SUPPLIER, owner_id=owner_id).all()


def get_suppliers_full_details(owner_id):
    return Organization.query.join(Addressbook, Organization.address_id == Addressbook.id) \
        ...
        .filter(Organization.owner_id == owner_id,
                Organization.org_type == organization_type.SUPPLIER).all()

def get_supplier_by_id(supplier_id, owner_id):
    return Organization.query.filter_by(org_type=organization_type.SUPPLIER, id=supplier_id,
                                        owner_id=owner_id).first()

def get_all_clients(owner_id):
    return Organization.query.filter_by(org_type=organization_type.CLIENT, owner_id=owner_id).all()

def get_all_affiliates(owner_id):
    return Organization.query.filter_by(org_type=organization_type.AFFILIATE, owner_id=owner_id).all()
...
def get_clients_full_details(owner_id):
    return Organization.query.join(Addressbook, Organization.address_id == Addressbook.id) \
        ...
        .add_columns(Addressbook, BankingDetails) \
        .filter(Organization.owner_id == owner_id,
                Organization.org_type ==  organization_type.CLIENT).all()

def get_project_organization(owner_id):
    return Organization.query.filter_by(org_type=organization_type.PROJECT, owner_id=owner_id).first()
